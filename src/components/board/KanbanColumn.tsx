...
export const KanbanColumn = ({ title, issues, onStatusChange, onCreateIssue, canMoveIssues }: KanbanColumnProps) => {
  ...
  const useVirtualization = issues.length > 20
  ...
  const columnClasses = `rounded-lg border-2 ${getColumnColor(title)} flex flex-col h-full max-h-[calc(100vh-250px)] transition-colors ${
    isOver ? "border-blue-500 dark:border-blue-400 bg-blue-100 dark:bg-blue-900/30" : ""
  }`

  return (
      <div className={columnClasses} ref={setNodeRef}>
        <div className={`px-4 py-3 rounded-t-lg ${getHeaderColor(title)} flex justify-between items-center`}>
