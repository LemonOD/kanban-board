...
export const IssueCard = ({ issue, onStatusChange, canMoveIssues }: IssueCardProps) => {
  ...
  const handleStatus = (e:any,status:IssueStatus) => {
    e.stopPropagation()
    onStatusChange(issue.id, status)
  }

  const cardClasses = `bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 
  p-4 shadow-sm hover:shadow-md dark:hover:shadow-lg transition-shadow ${ isDragging ? "opacity-50 rotate-2 scale-105" : ""
  } ${canMoveIssues ? "cursor-grab active:cursor-grabbing" : ""}`

  return (
      <div onClick={handleCardClick} ref={setNodeRef} 
                ...
                <div className="flex justify-between pt-2 border-t border-gray-100 dark:border-gray-700">
                  {previousStatus && (
                      <button
                          ...
                          className="px-3 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-200 
